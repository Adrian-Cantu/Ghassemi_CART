...
createEpiGenomicHeatMapData <- function(d, Rscript_path = '/home/opt/R-3.4.0/bin/Rscript', 
                                        softwarePath = '~/ext/epigeneticHeatmapMaker/epi_heatmap_from_file.R',
                                        controlFile = '~/ext/genomicHeatMapMaker/INSPIIRED.yml',
                                        maxSites = 25000, outputDir = NULL){
  ...
  comm <- paste(Rscript_path, softwarePath,  'samples',  
                '-c ', controlFile,
                '-t epiCellTypes',
                '-o', outputDir,
                '-f sites ',
...
createGenomicHeatMapData <- function(d, Rscript_path = '/home/opt/R-3.4.0/bin/Rscript', 
                                     softwarePath = '~/ext/genomicHeatMapMaker/genomic_heatmap_from_file.R',
                                     controlFile = '~/ext/genomicHeatMapMaker/INSPIIRED.yml',
                                     maxSites = 25000, outputDir = NULL){
  if(is.null(outputDir)){
    stop('Output dir not defined.')
