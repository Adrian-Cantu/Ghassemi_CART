...
dd$cellType <- gsub('^\\s+|\\s+$', '', toupper(d$cellType))

dd <- dplyr::group_by(d, timePoint, cellType) %>%
...
dd$posidLabel <- paste0(d$labeledNearestFeature, '\n', d$posid)
numClones <- 10
...
The ROC curve heatmaps were generated as described in @berry2006. They show how much the distribution of insertion sites across several genomic and epigenomic features differs from a random distribution. The heatmaps include D7 and D9 samples as well as some preinfusion samples from @nobles2020. All samples fallow roughly the same pattern for all features.
